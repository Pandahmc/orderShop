...
export default {
  ...
  computed: {
    positiveSize() {
      let count = 0
      this.ratings.forEach((rateTypes) => {
        if(rateTypes.rateType == 0){
          count ++
        }
      })
      return count
  ...
  created() {
  	...
  	this.ratings = FoodData.ratings,
    this.$nextTick(() => {
      this.scroll = new BScroll('.rating-wrapper',{
        click: true
      })
    })
  },
